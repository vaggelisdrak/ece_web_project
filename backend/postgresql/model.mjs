...
export let addTechnician = async (technician) => {
    ...
    try {
        ...
        const res = await client.query('SELECT MAX(id) FROM "Technician"');
        const maxId = res.rows[0].max;

        // Increment the maximum ID by 1 and use that as the ID for the new technician
        const newId = maxId ? maxId + 1 : 1;

        const query = 'INSERT INTO "Technician" (id, full_name, email, phone_number, specialty) VALUES ($1, $2, $3, $4, $5)';
        await client.query(query, [newId, technician.name, technician.email, technician.contact, technician.specialty]);
